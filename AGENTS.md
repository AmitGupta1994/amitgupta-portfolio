...
Seeding a remote database needs `NODE_ENV=production DATABASE_URL="…" npm run seed` — without `NODE_ENV=production` Payload treats it as a dev database and pushes schema, which makes the next `payload migrate` stop on a confirmation prompt. That prompt comes from `@payloadcms/drizzle` (triggered by a `batch = -1` row in `payload_migrations`) and **ignores** `--force-accept-warning`, so the `build` script answers it on stdin (`printf 'y\n' | payload migrate`). Without that a deploy hangs until it times out.
...
**One app, several sites, no `src/app/layout.tsx`.** Each route group owns a root layout: `src/app/(frontend)` is the main portfolio, `src/app/(research)` is the research site (`/research`), `src/app/(trek)` is the trekking site (`/trek`), `src/app/(payload)` is Payload's admin/API. Adding `src/app/layout.tsx` would break all of them. The site layouts share `src/components/SiteShell.tsx` (fonts, pre-paint theme script, `PageLoader`, `SmoothScroll`, `NavMenu`) and differ only in the navigation and metadata they pass in.

**Subdomains map to routes in `src/middleware.ts`.** `SITE_BY_SUBDOMAIN` maps `research.` → `/research` and `trek.` → `/trek`; the matcher skips `admin`, `api`, `_next` and files, so the CMS is reachable on every host. `/research` also works on the main domain. Adding a site: a row in that map, a route group, a Vercel domain and a DNS record.

**Per-site content is curated in the CMS, not duplicated.** `placementsField` (`src/cms/payload/fields/placements.ts`) adds `placements: [{ site, order }]` to Projects, Experiences, Expertise, SkillCategories and Publications. The main portfolio ignores placements and keeps using `order`/`priority`; other sites pass a `SiteKey` to the getters and `forSite()` (`src/cms/payload/adapter/site.ts`) keeps the items placed on that site, ordered by that placement. `SHOW_ALL_WHEN_UNCURATED` there decides the empty case per site: research falls back to everything (it draws on the same career), trek returns nothing, so unrelated coding content can never leak onto it — trek pages hide sections that come back empty.

**Media collections are scoped to one site each**, since tagging a 50-photo gallery item by item is miserable: `Photos` (an upload collection, Blob-backed in production) and `Videos` (a YouTube URL only — no hosting cost) each carry a single `site` select plus `album` and `order`. `src/cms/payload/adapter/videos.ts` parses the id out of any YouTube link shape and derives the thumbnail; the galleries render thumbnails that link out rather than embedding iframes.

**The research site has its own design, not shared chrome.** `src/app/(research)/layout.tsx` is a standalone document (no `SiteShell`, no theme toggle, loader or smooth-scroll): light only, Hanken Grotesk + Crimson Text via `next/font`, and `data-site="research"` on `<html>`. `globals.css` scopes that site's palette and `.dot-grid` under `[data-site="research"]`, and `--font-sans`/`--font-serif` resolve through `--font-site-sans`/`--font-site-serif` so each site can pick its own type without touching the others. Its components live in `src/components/research/` (masthead, `YearSection` with sticky year column, publication and role entries, labelled lists) and are not used by the portfolio. `src/lib/year.ts` turns the CMS's free-text dates ("2024, September", "08/08/2023 - 08/12/2024") into years for grouping.

**Cross-site links use `profile.mainSiteUrl`** (a CMS field), because a relative `/` on a subdomain is rewritten back to that same site by the middleware.

**Each standalone site has a global** built by `createSiteGlobal()` (`src/cms/payload/globals/siteGlobal.ts`): title, tagline, optional hero override, About HTML, its own nav and SEO. `getSitePage(site)` reads it, and `heroForSite()` (`src/lib/siteHero.ts`) applies the override or reuses the main hero. Adding `creatives` is: a `createSiteGlobal` call, a `SITE_OPTIONS` entry, a `SiteKey` value, a route group and a middleware row.
...
**Server-first components.** Sections are server components unless they need hooks; `MediumArticlesSection` is an async server component that fetches the Medium RSS feed through `api.rss2json.com` with `next: { revalidate: 86400 }` (keep it aligned with the page's `revalidate` — Next uses the shortest window on the route) and returns `null` on failure. Remote image hosts must be allowlisted in `next.config.ts` `images.remotePatterns` (Vercel Blob is already allowed); CMS uploads are covered by `localPatterns`.
