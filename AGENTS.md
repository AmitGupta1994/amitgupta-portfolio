...
**One app, several sites, no `src/app/layout.tsx`.** Each route group owns a root layout: `src/app/(frontend)` is the main portfolio, `src/app/(research)` is the research site (`/research`), `src/app/(payload)` is Payload's admin/API. Adding `src/app/layout.tsx` would break all of them. The site layouts share `src/components/SiteShell.tsx` (fonts, pre-paint theme script, `PageLoader`, `SmoothScroll`, `NavMenu`) and differ only in the navigation and metadata they pass in.

**Subdomains map to routes in `src/middleware.ts`.** `SITE_BY_SUBDOMAIN` rewrites `research.<domain>/…` to `/research/…`; the matcher skips `admin`, `api`, `_next` and files, so the CMS is reachable on every host. `/research` also works on the main domain. Adding a site: a row in that map, a route group, a Vercel domain and a DNS record.

**Per-site content is curated in the CMS, not duplicated.** `placementsField` (`src/cms/payload/fields/placements.ts`) adds `placements: [{ site, order }]` to Projects, Experiences, Expertise, SkillCategories and Publications. The main portfolio ignores placements and keeps using `order`/`priority`; other sites pass a `SiteKey` to the getters and `forSite()` (`src/cms/payload/adapter/site.ts`) filters to the items placed on that site and sorts them by that placement's order — falling back to everything in default order when nothing is curated yet, so a new site renders before any tagging. The research site's own copy (title, tagline, About HTML, nav, SEO) lives in the `Research` global.
